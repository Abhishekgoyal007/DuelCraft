...
contract DuelCraftCharacter is ERC721URIStorage, Ownable {
    ...
    // Mapping from character type to owner address (ensure each character type is unique)
    ...
    function mintCharacter(
        ...
        // Map character type to owner (lock this character type)
    ...
    /**
     * @dev Check if a character type is available for minting
     * @param characterType The character type to check
     * @return bool Whether the character is available (true) or already minted (false)
     */
    function isCharacterAvailable(string memory characterType) external view returns (bool) {
        return characterTypeOwner[characterType] == address(0);
    }
    
    /**
     * @dev Get the owner of a specific character type
     * @param characterType The character type to query
     * @return address The owner address (address(0) if not minted)
     */
    function getCharacterTypeOwner(string memory characterType) external view returns (address) {
        return characterTypeOwner[characterType];
    }
